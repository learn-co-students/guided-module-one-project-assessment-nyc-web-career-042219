...
class Game < ActiveRecord::Base
  ...
  def set_question_data(attributes)
    @question_data = call_trivia_api(attributes)
  end
  ...
  def call_trivia_api(amt = 10)
    res = JSON.parse(RestClient.get("https://opentdb.com/api.php?amount=#{amt}"))
    res['results']
  end
